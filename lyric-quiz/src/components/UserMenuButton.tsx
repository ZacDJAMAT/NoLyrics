...
// 1. On ajoute une prop pour écraser le comportement par défaut
...
export default function UserMenuButton({ onClickOverride }: UserMenuButtonProps) {
    ...
    // 2. La fonction de clic intelligente
    const handleProfileClick = () => {
        if (onClickOverride) {
            onClickOverride(); // Si on donne une action, on l'utilise (ex: ouvrir le calque)
        } else {
            navigate('/profile'); // Sinon, comportement normal
        }
    };
    ...
    if (user) {
        ...
        return (
            <Button
                variant="outline"
                size="icon"
                onClick={handleProfileClick} // 3. On utilise notre nouvelle fonction ici
                title="Mon Profil"
